...
static void usage(void) {
  fprintf(stderr, "heatshrink version %u.%u.%u by %s\n", version_major,
          version_minor, version_patch, author);
  fprintf(stderr, "Home page: %s\n\n", url);
  fprintf(
      stderr,
      "Usage:\n"
      "  heatshrink [-h] [-e|-d] [-v] [-w SIZE] [-l BITS] [IN_FILE] "
      "[OUT_FILE]\n"
      "\n"
      ...
      " -d        decode (decompress)\n"
      " -v        verbose (print input & output sizes, compression ratio, "
      "etc.)\n"
      "\n"
      " -w SIZE   Base-2 log of LZSS sliding window size\n"
      "\n"
      "    A larger value allows searches a larger history of the data for "
      "repeated\n"
...
static io_handle *handle_open(char *fname, IO_mode m, size_t buf_sz) {
  ...
  if (m == IO_READ) {
    ...
    if (0 == strcmp("-", fname)) {
      ...
      io->fd = open(fname, O_WRONLY | O_BINARY | O_CREAT | O_TRUNC /*| O_EXCL*/,
                    0644);
    }
  }
...
static ssize_t handle_read(io_handle *io, size_t size, uint8_t **buf) {
  ...
  if (rem >= size) {
    *buf = &io->buf[io->read];
    ...
    if (read_sz < 0) {
      HEATSHRINK_ERR(1, "read");
    }
    io->total += read_sz;
    if (read_sz == 0) { /* EOF */
      if (close(io->fd) < 0) {
        HEATSHRINK_ERR(1, "close");
      }
      io->fd = -1;
    }
    io->fill += read_sz;
...
static int encoder_sink_read(config *cfg, heatshrink_encoder *hse,
  ...
  do {
    if (data_sz > 0) {
      sres =
          heatshrink_encoder_sink(hse, &data[sunk], data_sz - sunk, &sink_sz);
      if (sres < 0) {
        die("sink");
      }
      sunk += sink_sz;
...
static int encode(config *cfg) {
  ...
  heatshrink_encoder *hse =
      heatshrink_encoder_alloc(window_sz2, cfg->lookahead_sz2);
  if (hse == NULL) {
...
static int decoder_sink_read(config *cfg, heatshrink_decoder *hsd,
  ...
  do {
    if (data_sz > 0) {
      sres =
          heatshrink_decoder_sink(hsd, &data[sunk], data_sz - sunk, &sink_sz);
      if (sres < 0) {
        die("sink");
      }
      sunk += sink_sz;
...
static int decode(config *cfg) {
  ...
  heatshrink_decoder *hsd =
      heatshrink_decoder_alloc(ibs, window_sz2, cfg->lookahead_sz2);
  if (hsd == NULL) {
    die("failed to init decoder");
  }
...
static void report(config *cfg) {
  ...
  fprintf(cfg->out->fd == STDOUT_FILENO ? stderr : stdout,
          ...
          100.0 - (100.0 * outb) / inb, inb, outb, cfg->window_sz2,
          cfg->lookahead_sz2);
}
...
int main(int argc, char **argv) {
  ...
  if (cfg.in == NULL) {
    die("Failed to open input file for read");
  }
  cfg.out = handle_open(cfg.out_fname, IO_WRITE, cfg.buffer_size);
  if (cfg.out == NULL) {
    die("Failed to open output file for write");
  }
