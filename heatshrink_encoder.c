...
static const char *state_names[] = {
    "not_full",      "filled",         "search",          "yield_tag_bit",
    "yield_literal", "yield_br_index", "yield_br_length", "save_backlog",
    "flush_bits",    "done",
};
#else
#define LOG(...)  /* no-op */
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start,
                                   uint16_t end, const uint16_t maxlen,
  ...
  *output_size = 0;
  ...
  while (1) {
    LOG("-- polling, state %u (%s), flags 0x%02x\n", hse->state,
        state_names[hse->state], hse->flags);
...
static HSE_state st_step_search(heatshrink_encoder *hse) {
  ...
  uint16_t match_pos =
      find_longest_match(hse, start, end, max_possible, &match_length);
  ...
  if (match_pos == MATCH_NOT_FOUND) {
    ...
    ASSERT(match_pos <=
           1 << HEATSHRINK_ENCODER_WINDOW_BITS(hse) /*window_length*/);
...
static uint16_t find_longest_match(heatshrink_encoder *hse, uint16_t start,
  ...
  const size_t break_even_point = (1 + HEATSHRINK_ENCODER_WINDOW_BITS(hse) +
